...
def medal_tally(df):
    mt = df.drop_duplicates(subset=['Team','NOC','Games','Year','City','Sport','Event','Medal'])
    mt = mt.groupby('region').sum()[['Gold','Silver','Bronze']].sort_values('Gold',ascending=False).reset_index()
...
def participating_nation(df,col):
    nations = df.drop_duplicates(['Year',col])['Year'].value_counts().reset_index().sort_values('index')
    nations.rename(columns={'index':'Editions','Year':'No Of '+col},inplace=True)
